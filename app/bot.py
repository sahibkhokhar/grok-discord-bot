...
async def query_xai_api_stream(context_messages: str, question: str) -> AsyncGenerator[str, None]:
    ...
    messages = [
        {
            "role": "system",
            "content": f"{PROMPT}\n\nYou are in a Discord conversation. Respond naturally and directly to the user's question. Do not repeat the message format or echo usernames in your response."
        },
        {
            "role": "user",
...
async def query_openai_api_stream(context_messages: str, question: str) -> AsyncGenerator[str, None]:
    ...
    try:
        stream = openai_client.responses.create(
            model=OPENAI_MODEL,
            input=user_input,
            instructions=f"{PROMPT or 'You are a helpful assistant.'}\n\nYou are in a Discord conversation. Respond naturally and directly to the user's question. Do not repeat the message format or echo usernames in your response.",
            stream=True,
        )
...
async def on_message(message):
    ...
    if message.author.id in BLOCKED_USER_IDS:
        print(f"Blocked user {message.author.name} (ID: {message.author.id}) tried to use the bot")
        return
    ...
    print(f"context to be sent to api:\n{context_for_grok}")
